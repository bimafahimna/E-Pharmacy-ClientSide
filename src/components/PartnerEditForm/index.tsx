...
import { getPartnersPaginated, putPartner } from "../../stores/PartnerReducer";

export interface partnerFormProps {
  onRequestClose: React.Dispatch<React.SetStateAction<boolean>>;
}

const PartnerEditForm: React.FC<partnerFormProps> = ({ onRequestClose }) => {
  const navigate = useNavigate();
  const AppDispatch = useAppDispatch();
  const [partnerForm, dispatchPartner, _, formattedEditRequest, isInputValid] =
  ...
  const handlePost = async () => {
    try {
      await AppDispatch(putPartner(formattedEditRequest())).unwrap();
  ...
  useEffect(() => {
    if (data === null) return;
    constants.PartnerEditRequestKeys.forEach((key) => {
      if (key === "operational_stop" || key === "operational_start") {
        dispatchPartner({
          type: "onchange",
          field: key,
          value: data[key].toString().replace("+07", "").replace("+7", ""),
        });
        return;
      }
      dispatchPartner({
